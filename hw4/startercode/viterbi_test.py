...
def run_viterbi_test():
    ...
    maxL = 5 # maximum number of labels (min is 2)
    ...
    for t in range(num_tests):
        ...
        # Generate the scores
        emission_scores = random.normal(0.0, emission_var, (B, N, L))
        trans_scores = random.normal(0.0, trans_var, (B, L, L))
        start_scores = random.normal(0.0, trans_var, (B, L))
        end_scores = random.normal(0.0, trans_var, (B, L))

        # run viterbi
        viterbi_s, viterbi_y = run_viterbi(emission_scores, trans_scores, start_scores, end_scores)
        # squeeze first dimension
        viterbi_y = viterbi_y[0]
        vitebi_s = viterbi_s[0]
        emission_scores = emission_scores.squeeze(axis=0)
        trans_scores = trans_scores.squeeze(axis=0)
        start_scores = start_scores.squeeze(axis=0)
        end_scores = end_scores.squeeze(axis=0)
