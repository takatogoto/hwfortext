...
class Feats:
    ...
    def index_data(self, sents, max_len=0):
        ...
        for s in sents:
            if max_len == 0:
                idxs.append(self.index_sent(s, len(s)))
            else:
                idxs.append(self.index_sent(s, max_len))
        self.freeze()
        assert len(idxs) == len(sents)
        return idxs

    def pad(self, sent, max_len):
        orig_len = len(sent)
        if len(sent) < max_len:
            sent += ['<PAD>'] * (max_len - len(sent))
        if len(sent) > max_len:
            sent = sent[:max_len]
        return sent, orig_len

    def index_sent(self, sent, max_len):
        """Compute and index the features of a single sentence."""
        sentIdxs = []
        sent, orig_len = self.pad(sent, max_len)
        for i in range(len(sent)):
            tokIdxs = []
            ftrs = self.token2features(sent, i, orig_len)
            for ftr in ftrs:
    ...
    def token2fidxs(self, sent, i):
        ...
        ftrs = self.token2features(sent, i, original_len)
        fidxs  = []
        for ftr in ftrs:
            if ftr in self.fmap:
